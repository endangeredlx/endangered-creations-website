...
function confirm_delete_entry() 
{
   ...
   var base = getBaseURL();
   var posturl = ( base + "/downloads/remove/" + did );
   ...
   var buttons = {
      ...
      option2  :  {
         id          : 'confirm_button',
         title       : 'Delete',
         onConfirm   : 'delete_item_entry',
         class       : 'light_blue_button',
         args        : myArgs
      }
   }; 
   chi_confirm( 'Are you sure that you want to delete \''+ $(this).parent().find("div.edit_long a").html() +'\'? This cannot be undone.', 'Delete Download', buttons );
}

function delete_item_entry( obj ) 
{
   $.ajax({
		url: obj.url,
		type: 'POST',
...
$(document).ready( function() {
	$('.d_downloads').click( confirm_delete_entry );
});
