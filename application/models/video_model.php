...
class Video_model extends CI_Model 
{
    ...
    public function get_playlists( $args )
    {
        ...
        if( isset( $args['id'] ) )
        {
            ...
            $query = $this->db->get( 'video_playlists' );
        }
        else if( $args['how_many'] != 'all' )
        {
            $query = $this->db->get( 'video_playlists', $args['how_many'], $args['offset'] );
        }
        else 
        {
            $query = $this->db->get( 'video_playlists' );
    ...
    public function get_videos( $args )
    {
        ...
        {
            $query = $this->db->get( 'video_videos', $args['how_many'], $args['offset'] );
        }
        else 
        {
            $query = $this->db->get( 'video_videos' );
    ...
    public function make_row()
    {
        $this->db->insert( 'video_songs', array( 'title' => 'title' ) );
    ...
    public function add_playlist( $data ) 
    {	
        $this->db->insert( 'video_playlists', $data );
        return true;
    }
    // }}}
    ...
    public function create_playlist_records( $id, $data )
    {
        foreach( $data as $song )
        {
            ...
            $this->db->insert( 'video_records', $insert );
        }
        return true;
    }
    // }}}
    ...
    public function update_order()
    {
        ...
        foreach( $songs->result() as $song )
        {
            ...
            $this->db->update( 'video', array( 'order' => $count ) ); 
            $count++;
        }
        return true;
    }
    ...
    {
        $this->db->where( 'id', $id );
        $this->db->update( 'video_songs', $data );
        return true;
    }
    // }}}
    ...
    {
        $this->db->where( 'id', $id );
        $this->db->update( 'video_playlists', $data );
    }
    // }}}
    ...
    public function update_playlist_records( $id, $data )
    {
        ...
        foreach( $data as $record )
        {
            ...
            $query = $this->db->get( 'video_records' );
            ...
            { 
                ...
                $this->db->where( array( 'order' => $record['order'], 'playlist_id' => $id ) );
                $up = $this->db->update( 'video_records', $update );
                if( ! $up ) echo 'failure';
            }
            else
            {
                ...
                $add = $this->db->insert( 'video_records', $insert );
                if( ! $add ) echo 'failure';
            }
        }
    }
    ...
    {	
        $this->db->where( 'id', $id );
        $this->db->delete( 'video_playlists' );
        $this->db->flush_cache();
        $this->db->where( 'playlist_id', $id );
        $this->db->delete( 'video_records' );
    }
    // }}}
}
